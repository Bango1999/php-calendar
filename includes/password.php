        ...
        switch ($algo) {
            case PASSWORD_BCRYPT:
                ...
                if (isset($options['cost'])) {
                    $cost = $options['cost'];
                    if ($cost < 4 || $cost > 31) {
                        trigger_error(sprintf("password_hash(): Invalid bcrypt cost parameter specified: 
...
PasswordCompat\binary\_strlen($salt), $required_salt_len), E_USER_WARNING);
    ...
    function password_get_info($hash) {
        ...
        if (PasswordCompat\binary\_substr($hash, 0, 4) == '$2y$' && PasswordCompat\binary\_strlen($hash) == 
60) {
